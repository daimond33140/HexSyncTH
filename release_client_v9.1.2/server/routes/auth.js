...
router.post('/request-otp', async (req, res) => {
  try {
    ...
    // In a production server, this would send via nodemailer. Here we simulate and return it for convenience!
    console.log(`[OTP Generated] For ${email}: ${otp}`);
    res.json({
      message: 'ส่งรหัส OTP ไปยัง Gmail ของคุณเรียบร้อยแล้ว (มีอายุ 5 นาที)',
      mockOtp: otp // sent to client for seamless instant demonstration
    });
  } catch (err) {
