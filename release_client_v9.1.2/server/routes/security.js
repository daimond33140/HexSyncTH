...
router.post('/report-threat', async (req, res) => {
  try {
    ...
    let strikes = Number(strikeCount) || 1;

    // Check recent threat history for this entity
    const Op = require('sequelize').Op;
    const whereConditions = [{ ip }];
    if (deviceId && deviceId.trim()) whereConditions.push({ deviceId: deviceId.trim() });
    if (username && username !== 'Anonymous') whereConditions.push({ username });
    ...
    // RULE: If the entity is NOT currently banned, BUT their previous threat record was banned (banned === true):
    // This proves they were previously banned and have now been unbanned!
    // So reset their strike counter back to 1 (1/3)!
...
router.post('/verify-passcode', async (req, res) => {
  try {
    ...
    return res.status(401).json({ valid: false, message: 'รหัสผ่านความปลอดภัยไม่ถูกต้อง' });
  } catch (err) {
