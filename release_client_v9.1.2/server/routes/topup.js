...
router.post('/bank-config', requireAdmin, async (req, res) => {
  try {
    const { bank_name, bank_account_name, bank_account_number, promptpay_number, adminUsername } = req.body;
    const updates = { bank_name, bank_account_name, bank_account_number, promptpay_number };
...
router.post('/bank-slip', async (req, res) => {
  try {
...
router.post('/angpao', async (req, res) => {
  try {
...
// 3. Confirm Dynamic QR Payment (Single-use redemption & auto balance credit)
  ...
  try {
    ...
    if (order.status === 'paid') {
      return res.json({
        success: true,
        alreadyPaid: true,
        message: 'รายการนี้ได้รับการยืนยันและเติมเงินเข้าสู่ระบบแล้ว (Single-Use QR ใช้งานเสร็จสิ้น)',
        amount: order.amount,
      });
    }
    ...
    // If slipImage is provided, run anti-duplicate slip checks to prevent reused slips
    let slipTxId = null;
    let transRef = null;
    if (slipImage && typeof slipImage === 'string' && slipImage.length > 50) {
      ...
      const slipTx = await SlipTransaction.create({
        ...
        verifiedVia: 'dynamic_qr_verified',
        status: 'approved',
      });
      slipTxId = slipTx.id;
    }
    ...
    await Log.create({
      ...
      detail: `ผู้ใช้ ${targetUsername} เติมเงินผ่าน PromptPay Dynamic QR (เลขที่บิล: ${order.orderId}) สำเร็จ +฿${addedAmount.toLocaleString()} บาท (เดิม ฿${previousBalance.toLocaleString()} -> ใหม่ ฿${user.creditBalance.toLocaleString()}) [QR ใช้ครั้งเดียว 30 นาที]`,
      username: targetUsername,
...
router.get('/history', async (req, res) => {
  try {
    ...
    const targetUser = username ? String(username).trim() : null;
